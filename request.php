...
class Request
{
    ...
    public function setInsertAviateur($nom, $prenom, $pseudo, $naissance, $mail, $mdp)
    {
        ...
        $query = "INSERT INTO candidat( nom, prenom, pseudo, naissance, mail, mdp ) VALUES ('{$nom}', '{$prenom}', '{$pseudo}','{$naissance}','{$mail}', '{$mdp}')";
    ...
    //fonction pour creer une nouvelle annonce

    public function setInsertoffre($nomA, $nomE, $lieux, $debut, $fin, $salaire, $descri)
    {

        
        $query = "INSERT INTO offres( nomA, nomE, lieux, debut, fin, salaire, descri ) VALUES ('{$nomA}','{$nomE}','{$lieux}', {$debut}','{$fin}','{$salaire}', '{$descri}')";
        $req = $this->bdd->prepare($query);
        echo $query;
        $req->execute();
        return true;
    ...
    {
        $req = $this->bdd->prepare("SELECT mdp FROM candidat WHERE pseudo ='{$pseudo}';");
        $req->execute();
        $resultat = $req->fetch();
